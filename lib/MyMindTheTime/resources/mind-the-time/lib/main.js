...
var mtt_clock_on = function(theDmn, theProt, thePath, theTitle, theThumbnailURI) {
	...
	if (legitURL.test( tabs.activeTab.url )) {
		ticker.port.emit("tickerOn");
            ...
            tmpThumbnailURI = theThumbnailURI;
	}
...
var mtt_clock_off = function() {
    ...
    var Y = tmpStartTime.getFullYear();
    var M;
    if((tmpStartTime.getMonth() + 1).toString().length == 1){
        M = "0" + (tmpStartTime.getMonth() + 1).toString()
    }else{
        M = tmpStartTime.getMonth() + 1;
    }
    var D;
    if((tmpStartTime.getDate()).toString().length == 1){
        D = "0" + (tmpStartTime.getDate()).toString()
    }else{
        D = tmpStartTime.getDate();
    }
    var h;
    if((tmpStartTime.getHours()).toString().length == 1){
        h = "0" + (tmpStartTime.getHours()).toString()
    }else{
        h = tmpStartTime.getHours();
    }
    var m;
    if((tmpStartTime.getMinutes()).toString().length == 1){
        m = "0" + (tmpStartTime.getMinutes()).toString()
    }else{
        m = tmpStartTime.getMinutes();
    }
    var s = tmpStartTime.getSeconds();
    if((tmpStartTime.getSeconds()).toString().length == 1){
        s = "0" + (tmpStartTime.getSeconds()).toString()
    }else{
        s = tmpStartTime.getSeconds();
    }
    mDBConn.executeSimpleSQL("INSERT INTO log VALUES('" + tmpTitle + "', '" + tmpURL +
                             "', datetime('" + Y +
                             "-" + M +
                             "-" + D +
                             " " + h +
