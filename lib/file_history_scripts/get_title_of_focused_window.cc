...
static const int kNumberBufferSize = 255;

int GetFocusedWindowInfo(std::string *title, std::string *filename, long *number) {
  *title = "";
  *filename = "";
  ...
  for (int i = 0; i < CFArrayGetCount(windows); i++) {
    ...
    CFNumberRef window_owner_pid = (CFNumberRef)CFDictionaryGetValue(dict, kCGWindowOwnerPID);
    int window_owner_pid_int;
    CFNumberGetValue(window_owner_pid, kCFNumberIntType, &window_owner_pid_int);

    ProcessSerialNumber owner_serial_number;
    GetProcessForPID(window_owner_pid_int, &owner_serial_number);

    Boolean are_same;
    OSStatus err = SameProcess(&front_process_serial_number, &owner_serial_number, &are_same);
    if (err) {
      continue;
    }

    if (TRUE == are_same) {
      CFNumberRef window_number = (CFNumberRef)CFDictionaryGetValue(dict, kCGWindowNumber);
      if(window_number) {
        CFNumberGetValue(window_number, kCFNumberLongType, number);
      }
      CFStringRef window_name = (CFStringRef)CFDictionaryGetValue(dict, kCGWindowName);
      if (window_name) {
        char title_buffer[kTitleBufferSize];
        CFStringGetCString(window_name, title_buffer, kTitleBufferSize, kCFStringEncodingUTF8);
  ...
  CFStringGetCString(processName, filename_buffer, kFilenameBufferSize, kCFStringEncodingUTF8);
...
int main(){
  ...
  GetFocusedWindowInfo(&title, &filename, &number);
